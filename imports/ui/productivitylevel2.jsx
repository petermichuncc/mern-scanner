...
export default class ProductivityLevel2 extends Component {
...
axios.get('/departmentAllMinutesLostLevel2',{
  params: {
...
console.log("This is the department "+this.state.value)
   ...
   {
     //console.log("This is the workcenter "+datatest[i].workcenter)

   //datatest[0].workcenter is the workcenter
   //console.log("This is the amount "+datatest[i].amount)
   // shift1.push({ y: parseInt(testarray[i][1]), color: '#ffc107' });

   prodValue.push({ y: parseInt(datatest[i].amount), color: datatest[i].color, name:datatest[i].name, workcenter:datatest[i].workcenter, partnumber:datatest[i].partnumber })
   ...
   prodValue.sort(compare);
   prodValue=prodValue.slice(0,10)

   for (var i=0; i<prodValue.length;i++)
   {
     //console.log("This is the workcenter "+datatest[i].workcenter)
   //workcenters.push(datatest[i].workcenter)
   console.log("this is the prodvalue name "+ prodValue[i].name)
   prodIssues.push(prodValue[i].name+" - "+prodValue[i].workcenter+" - "+prodValue[i].partnumber )
   //datatest[0].workcenter is the workcenter
   //console.log("This is the amount "+datatest[i].amount)
   // shift1.push({ y: parseInt(testarray[i][1]), color: '#ffc107' });
   //prodValue.push({ y: parseInt(datatest[i].amount), color: datatest[i].color, data:datatest[i].name })
   }
...
var titleFont='20px'
 ...
 var config = {
  ...
  chart: {
     ...
     title: {
       margin: 70,
         text: 'Productivity Level 2: '+department,
           style: {
                   fontWeight: 'bold',
                     color: 'black',
                     fontSize:titleFont
